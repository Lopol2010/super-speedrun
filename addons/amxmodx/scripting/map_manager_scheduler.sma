...
public plugin_init()
{
    ...
    g_pCvars[INTERMISSION_DELAY] = register_cvar("mapm_intermission_delay", "10"); // seconds
...
public mapm_vote_finished(const map[], type, total_votes)
{
    ...
    if(get_num(LAST_ROUND)) {
        ...
        new delay = get_float(INTERMISSION_DELAY) > 0.0 ? floatround(get_float(INTERMISSION_DELAY)) : get_num(CHATTIME);
        client_print_color(0, print_team_default, "%s^1 %L^1 %L.", g_sPrefix, LANG_PLAYER, "MAPM_MAP_CHANGE", delay, LANG_PLAYER, "MAPM_SECONDS");
        set_task(get_float(INTERMISSION_DELAY), "delayed_intermission", TASK_DELAYED_INTERMISSION);
