/* 
    ...
    TODO:
        ...
        - [ ] При открытии top15 вначала на мгновенье видна надпись "Loading..." раньше вроде проверял и такого не было, может кеширование слетело?
        - [ ] Античит. Сделать подсчет кадров за всё время прохождения карты, так можно будет отловить тех у кого кадров окажется больше чем должно быть на выбранной категории фпс.
        - [ ] Разброс фпс позволяет на 100фпс ставить 104
        - [ ] Сделать проверку косаний через boxes distance в box_system
        ...
        - [x] Важно! Если игрок запустил таймер и зашел в наблюдатели за игроком который на финише, то наблюдатель финиширует за 0 сек.
        - [x] Баг, позволяет выйти со старта не запуская таймер. Быстро сохраняешься в /save и с помощью +strafe выходишь со старта. 
        - [x] Проверить наличие уязвимости SQL Injection. (Нашёл и исправил. А побудила то меня, просьба о помощи от человека, но устранил его и у себя в спидран моде)
        - [x] шустрик в стиме написал что худ спектаторов обрезается, видимо ттолько если ты сам спектатор
            - [x] опять сказал обрезается ник, надо проверить
